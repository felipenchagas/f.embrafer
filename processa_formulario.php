...
// Verifica se há erro na conexão com BD1
if ($conexao1->connect_error) {
    die("Erro ao conectar ao primeiro banco de dados (BD1): " . $conexao1->connect_error);
}

// Verifica se há erro na conexão com BD2
if ($conexao2->connect_error) {
    die("Erro ao conectar ao segundo banco de dados (BD2): " . $conexao2->connect_error);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($erros)) {
        ...
        header('Location: index.html?erro=true'); // Alterado para index.html
        exit();
    }
    ...
    if ($stmt1 = $conexao1->prepare($sql)) {
        ...
        if ($stmt1->execute()) {
            ...
            error_log("Erro ao inserir dados no primeiro banco de dados (BD1): " . $stmt1->error);
        }
        $stmt1->close();
    } else {
        error_log("Erro ao preparar a consulta no primeiro banco de dados (BD1): " . $conexao1->error);
    }
    ...
    if ($stmt2 = $conexao2->prepare($sql)) {
        ...
        if ($stmt2->execute()) {
            ...
            error_log("Erro ao inserir dados no segundo banco de dados (BD2): " . $stmt2->error);
        }
        $stmt2->close();
    } else {
        error_log("Erro ao preparar a consulta no segundo banco de dados (BD2): " . $conexao2->error);
    }
    ...
    header('Location: index.html'); // Alterado para index.html
    exit();
}
?>
